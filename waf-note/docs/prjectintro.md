...
针对传统云服务器(CVM)集群存在扩缩容困难、运营成本⾼问题，进行了集群迁移上云项目。旨在将集群迁移到基于Kubernetes的容器管理服务(TKE)上，实现高度可扩展、成本控制的目标。

用到的技术栈用腾讯云DNS K8S VPC golang python nginx 
...
针对多个防护引擎因串行检测导致时延大的问题，进行了防护引擎性能优化项目。新增路由组件将串行优化为并行检测、优化检测逻辑、优化服务性能。最终减少整体检测时延。

技术层面设计 垃圾回收机制优化 编解码优化 内核参数优化 TencentRPC框架 基于协程并发编程 
...
- 使用读写锁代替互斥锁
- 使用atmoic代替加锁，atmoic底层基于CAS乐观加锁
- 使用并发安全的sync.map代替需要加锁的map
...
与微信安全网关合作，打造小程序安全加速产品。依托微信生态创造腾讯云WAF差异化竞争能力。

技术上设计nginx python 小程序安全网关
...
深入研究了开源Python框架Freqtrade的源码，并对框架进行了二次开发。熟练掌握量化框架搭建的基本流程，对量化框架在行情数据获取和清洗、回测逻辑、实盘交易和分析、传统策略开发、机器学习策略开发、超参数优化的实现细节进行了研究。
...
然后就是进行策略开发。策略开发的思路主要源于书籍、论坛、论文

#### 开发策略

##### 202310-202401 技术指标策略
...
##### 202405-202406 多因子对冲策略

在因子挖掘上，我尝试了常见的基本面因子和技术面因子。选择因子排名靠前的一组做多，因子排名靠后的一组做空。

通过回测查看收益是否具有单调性。具有单调性则因子有效。

获得多个因子后，按照收益协方差矩阵计算因子相关性，排除相关性较高因子。

在对剩下的因子进行等权或者非等全组合得到多因子模型。

最后按照模型对所有币种评分，做多高分币种，做空低分币种，定时换仓。理想情况下，该策略排除了市场系统性下跌风险，因子有效则可以一直盈利。
...
从做多候选池中打乱顺序选择5个币种开仓做多。
从做空候选池中打乱顺序选择5个币种开仓做空。
